...
func (c *RedisPushPull) PushMsgFnExp(ctx context.Context, queueName string, msg []byte, expiration time.Duration) error {
	rdb, err := GetRedisClient(c)
	if err != nil {
		return err
	}
	...
	// 设置过期时间
	err = rdb.Expire(ctx, queueName, expiration).Err()
	if err != nil {
		return err
	}
